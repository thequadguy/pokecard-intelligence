...
async def run_content_generation():
    ...
    try:
        ...
        try:                 response = client.models.generate_content(model='gemini-1.5-flash', contents=prompt)                 tweet_text = response.text.strip()             except Exception as gem_err:                 print(f"Gemini error: {gem_err}. Using template.")                 tweet_text = "The thrill of finding that rare Charizard is unmatched. Every card tells a story. Keep hunting, keep collecting. 🎛️ #TheRarePick"
    ...
    for item in watchlists:
        ...
        if data["found"] and data["price"] > 0:
            ...
            if data["price"] <= float(item["target_price"]):
                ...
                try:                     ai_try:                 response = client.models.generate_content(model='gemini-1.5-flash', contents=prompt)                 tweet_text = response.text.strip()             except Exception as gem_err:                 print(f"Gemini error: {gem_err}. Using template.")                 tweet_text = "The thrill of finding that rare Charizard is unmatched. Every card tells a story. Keep hunting, keep collecting. 🎛️ #TheRarePick"                     tweet_text = ai_response.text.strip()                 except Exception as gem_err:                     print(f"Gemini error: {gem_err}. Using template.")                     tweet_text_raw = f"🔥 DEAL ALERT! {item['card_name']} ({item['set_name']}) spotted at ${data['price']} - below our ${item['target_price']} target!"
