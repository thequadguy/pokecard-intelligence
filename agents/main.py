...
async def run_content_generation():
    ...
    try:
        prompt = (
            "You are the social media manager for 'The Rare Pick', a premium Pokemon card collecting brand. "
            "Write an engaging, authentic Twitter post (under 280 characters) about the thrill of collecting vintage Pokemon cards. "
        ...
        try:
            ...
            print(f"Gemini error: {gem_err}. Using template tweet.")
            tweet_text = "The thrill of finding that rare holographic card never gets old. Every pack is a mystery waiting to unfold. Keep hunting, collectors."
        print(f"\nGenerated Tweet:\n{tweet_text}\n")
    ...
    for item in watchlists:
        ...
        if data["found"] and data["price"] > 0:
            ...
            if data["price"] <= float(item["target_price"]):
                ...
                prompt = (
                    f"A Pokemon card deal has been found!\n"
